...
/**
 * REFACTORIZADO: La notificación ahora usa clases CSS en lugar de estilos en línea.
 */
function mostrarNotificacion(mensaje, tipo = 'success') {
    ...
    // Asignar la clase base y la clase específica del tipo
    noti.className = `notificacion-flotante notificacion-${tipo}`;

    // El contenido ahora es simple, la apariencia se controla con CSS
    noti.innerHTML = `
        <i class="fas ${tipo === 'success' ? 'fa-check-circle' : tipo === 'error' ? 'fa-exclamation-circle' : tipo === 'info' ? 'fa-info-circle' : 'fa-exclamation-triangle'}" style="font-size: 20px;"></i>
    ...
    // Asegurar que exista la animación (puede estar en el CSS principal)
    // La animación se define en el CSS, no en línea.

    // Auto-cerrar después de 5 segundos
    setTimeout(() => {
        if (noti.parentNode) {
            noti.style.animation = 'slideOutRight var(--transition-fast) forwards';
